use crate::routes::auth::Claims;
use actix_web::HttpRequest;
use jsonwebtoken::{decode, DecodingKey, Validation};
use std::collections::HashMap;
use std::env;
use thiserror::Error;

// ── Token extraction ──────────────────────────────────────────────────────────

/// Reads the `token` cookie from the Cookie header.
pub fn get_token_from_cookie(req: &HttpRequest) -> Option<String> {
    req.headers()
        .get(actix_web::http::header::COOKIE)
        .and_then(|header| header.to_str().ok())
        .and_then(|cookies| {
            cookies.split(';').find_map(|cookie| {
                let mut parts = cookie.trim().splitn(2, '=');
                if parts.next()? == "token" {
                    parts.next().map(String::from)
                } else {
                    None
                }
            })
        })
}

/// Reads a Bearer token from the standard `Authorization` header.
pub fn get_token_from_auth_header(req: &HttpRequest) -> Option<String> {
    req.headers()
        .get(actix_web::http::header::AUTHORIZATION) // was PROXY_AUTHORIZATION — bug
        .and_then(|header| header.to_str().ok())
        .and_then(|header_str| header_str.strip_prefix("Bearer ").map(String::from))
}

/// Returns the JWT from either the cookie or the Authorization header.
pub fn get_jwt_token(req: &HttpRequest) -> Option<String> {
    get_token_from_cookie(req).or_else(|| get_token_from_auth_header(req))
}

// ── Verification ──────────────────────────────────────────────────────────────

/// Decodes and validates the JWT, returning its claims as a flat string map.
pub async fn verify(
    req: HttpRequest,
) -> Result<HashMap<String, String>, AppError> {
    let token = get_jwt_token(&req).ok_or(AppError::TokenNotFound)?;

    let secret_key = env::var("SECRET_KEY").map_err(|_| AppError::SecretKeyNotSet)?;

    let token_data = decode::<Claims>(
        &token,
        &DecodingKey::from_secret(secret_key.as_ref()),
        &Validation::default(),
    )
    .map_err(|e| AppError::DecodeError(e.to_string()))?;

    let mut claims_map = HashMap::new();
    claims_map.insert("sub".to_string(), token_data.claims.sub);
    claims_map.insert("exp".to_string(), token_data.claims.exp.to_string());
    claims_map.insert(
        "is_admin".to_string(),
        token_data.claims.is_admin.to_string(),
    );

    Ok(claims_map)
}

// ── Errors ────────────────────────────────────────────────────────────────────

#[derive(Error, Debug)]
pub enum AppError {
    #[error("Token not found")]
    TokenNotFound,

    #[error("SECRET_KEY not set")]
    SecretKeyNotSet,

    #[error("Failed to decode token: {0}")]
    DecodeError(String),
}